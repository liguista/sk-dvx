...
/**
 	* A data access object (DAO) providing persistence and search support for Attribute entities.
 			* Transaction control of the save(), update() and delete() operations 
		can directly support Spring container-managed transactions or they can be augmented	to handle user-managed Spring transactions. 
		Each of these methods provides additional information for how to configure it for the desired type of transaction control. 	
	 * @see hibernate_dvx.Attribute
...
public class AttributeDAO extends BaseHibernateDAO  {
    ...
    public Attribute findById( hibernate_dvx.AttributeId id) {
        log.debug("getting Attribute instance with id: " + id);
        try {
            Attribute instance = (Attribute) getSession()
                    .get("hibernate_dvx.Attribute", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(Attribute instance) {
        ...
        try {
            List results = getSession()
                    .createCriteria("hibernate_dvx.Attribute")
    ...
    public Attribute merge(Attribute detachedInstance) {
        ...
        try {
            Attribute result = (Attribute) getSession()
