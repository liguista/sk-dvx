 ...
 * MovieMenu entities. Transaction control of the save(), update() and delete()
...
public class MovieMenuDAO extends BaseHibernateDAO {
	...
	public MovieMenu findById(java.lang.Integer id) {
		...
		try {
			MovieMenu instance = (MovieMenu) getSession().get(
					"org.ski.dvx.hibernate.MovieMenu", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public MovieMenu merge(MovieMenu detachedInstance) {
		...
		try {
			MovieMenu result = (MovieMenu) getSession().merge(detachedInstance);
